...
**Status:** Updated at the close of every roadmap phase (current: Phase 8 complete)
...
| **[Dividend Distributor](./13-dividend-distributor.md)** | Phase 8: the accumulator identity, the rounding rule that keeps it solvent, and the two bugs the suite found |

---

## 📊 Current Status

**303 tests, all green.** Every contract under `src/` is at 100% line, statement, branch and function coverage. The handler-based invariant suite is live, and found a real bug on its first high-effort run (see [Invariant Suite](./11-invariants.md)); the dividend suite found two more (see [Dividend Distributor](./13-dividend-distributor.md)).
...
| [`DividendDistributorTest`](../../test/unit/DividendDistributor.t.sol) | Unit + Fuzz | 41 | 8 |
...
| [`MaxHoldersModuleTest`](../../test/unit/MaxHoldersModule.t.sol) | Unit + Fuzz | 29 | 2 |
...
| [`DividendsTest`](../../test/scenario/Dividends.t.sol) | Scenario | 4 | 8 |
| **Total** | | **303** | |
...
| :--- | :---- | :--------- | :------- | :-------- |
| `src/SecurityToken.sol` | 100.00% (104/104) | 100.00% (124/124) | 100.00% (32/32) | 100.00% (18/18) |
...
| `src/compliance/modules/DividendDistributor.sol` | 100.00% (54/54) | 100.00% (60/60) | 100.00% (7/7) | 100.00% (15/15) |
...
| **Dividend solvency** | The distributor always holds at least what it reports as owed | [`testFuzz_solventAcrossRepeatedDeposits`](../../test/unit/DividendDistributor.t.sol#L567) over up to twenty deposits, plus the regression [`test_deposit_claimsNeverExceedFundsAcrossManyDeposits`](../../test/unit/DividendDistributor.t.sol#L255). **Not** in the handler suite: the distributor is not registered on the handler's system, so these hold under fuzzed sequences but not under arbitrary interleavings with freeze, pause and recovery. See [Gaps](./12-gaps-and-roadmap.md) |
| **Entitlement conserved by transfer** | A transfer changes who is owed, never the total owed | [`testFuzz_transferPreservesTotalEntitlement`](../../test/unit/DividendDistributor.t.sol#L549), plus the mint/burn one-sided cases in [Dividend Distributor](./13-dividend-distributor.md#correction-accounting) |
